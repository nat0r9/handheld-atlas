...
function reviewAge(value: string | null) {
  if (!value) {
      ...
      <div className="mx-auto max-w-7xl">
        ...
        <section className="mt-6 rounded-[2rem] border border-slate-800 bg-gradient-to-br from-slate-900 via-slate-950 to-black p-7 md:p-10">
          ...
          <p className="mt-4 max-w-3xl text-lg leading-8 text-slate-400">
            Review revision history, inspect evidence and publish community settings through a transaction-safe workflow.
          </p>
        </section>
        ...
        {submissions.length === 0 ? (
          <section className="mt-6 rounded-3xl border border-dashed border-slate-800 bg-slate-900/50 p-12 text-center">
            <h2 className="text-2xl font-black">
              No {getSubmissionStatusLabel(selectedStatus).toLowerCase()} submissions
            </h2>
            <p className="mt-3 text-slate-500">
              The queue is clear. Somewhere, a moderator has briefly achieved inner peace.
            </p>
          </section>
        ) : (
                  ...
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div className="min-w-0">
                      <p className="text-xs font-black uppercase tracking-[0.16em] text-cyan-400">
                        {relationName(submission.games)}
                      </p>
                      <h2 className="mt-2 break-words text-2xl font-black">
                        {submission.name}
                      </h2>
                      <p className="mt-2 text-sm text-slate-500">
                        {relationName(submission.handhelds)}
                      </p>
                    </div>
                    ...
                    <QueueStat
                      ...
                      value={
                        submission.status === "pending"
                          ? reviewAge(submission.submitted_at)
...
function QueueStat({
  ...
  return (
    <div className="min-w-0 rounded-xl border border-slate-800 bg-black/20 p-3">
