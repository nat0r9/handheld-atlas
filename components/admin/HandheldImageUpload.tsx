...
interface HandheldImageUploadProps {
  inputName?: string;
  defaultUrl?: string;
}

export default function HandheldImageUpload({
  inputName = "imageUrl",
  defaultUrl = "",
}: HandheldImageUploadProps) {
  ...
  return (
    <div>
      ...
      <label className="flex cursor-pointer items-center justify-center rounded-2xl border border-dashed border-slate-700 bg-slate-950 px-5 py-8 text-center transition hover:border-cyan-500">
        ...
        <span>
          <span className="block font-bold text-slate-200">
            {isUploading
              ? "Uploading image..."
              : "Choose handheld image"}
          </span>

          <span className="mt-2 block text-sm text-slate-500">
            Transparent PNG or WebP recommended · maximum 5 MB
          </span>
        </span>
      </label>
      ...
      {imageUrl && (
        <div className="mt-5 rounded-3xl border border-slate-800 bg-gradient-to-br from-slate-950 via-slate-900 to-black p-6">
          <div className="relative mx-auto h-56 w-full max-w-xl">
            <Image
              src={imageUrl}
              alt="Handheld preview"
              fill
              sizes="576px"
